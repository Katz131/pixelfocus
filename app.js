...
(() => {
  ...
  window.openTitleLadderModal = openTitleLadderModal;
  ...
  function generateCanvasUpgrades(currentSize) {
    ...
    upgrades.push({ size: 12, cost: 200,  dollarCost: 1000,  label: '12\u00d712' });
    upgrades.push({ size: 16, cost: 2000, dollarCost: 10000, label: '16\u00d716' });
  ...
  const DEFAULT_STATE = {
    ...
    distractionCategories: [],   // v3.23.311: user-defined distraction categories [{id,name,color}]
    sessionDistractions: {},     // v3.23.311: current session distraction counts {categoryId: count}
    distractionHistory: [],      // v3.23.311: past session distraction summaries
    ...
    // v3.23.192: Tutorial system — tracks which tutorial tiers are unlocked
    // Tiers 0-1 are always unlocked. Tiers 2-5 require HMAC codes.
    // Object maps tier number (string) to true. E.g. { '2': true, '3': true }
    tutorialUnlocked: {},
  };
  ...
  function load(cb) {
    chrome.storage.local.get(['pixelFocusState', 'pixelFocusState_backup_safe', 'pixelFocusState_backup_safe2', 'pixelFocusProfileId'], (result) => {
      ...
      if (_loaded) {
        ...
        if (state.focusHistory && Object.keys(state.focusHistory).length > 0) {
          var _rs226 = new Date();
          var _rs226Count = 0;
          // If user already focused today, count today
          if ((state.todayBlocks || 0) > 0) _rs226Count = 1;
          // Walk backwards from yesterday
          var _rs226Start = _rs226Count > 0 ? 1 : 1; // always start from yesterday
          for (var _rs226i = _rs226Start; _rs226i < 3650; _rs226i++) {
            var _rs226D = new Date(_rs226.getTime() - _rs226i * 86400000);
            var _rs226MM = _rs226D.getMonth() + 1;
            var _rs226DD = _rs226D.getDate();
            var _rs226Key = _rs226D.getFullYear() + '-' + (_rs226MM < 10 ? '0' : '') + _rs226MM + '-' + (_rs226DD < 10 ? '0' : '') + _rs226DD;
            if (state.focusHistory[_rs226Key] && state.focusHistory[_rs226Key] > 0) {
              _rs226Count++;
            } else { break; }
          }
          if (_rs226Count > (state.realStreak || 0)) {
            state.realStreak = _rs226Count;
            if (_rs226Count > (state.longestRealStreak || 0)) state.longestRealStreak = _rs226Count;
          }
        }
  ...
  function checkDayRollover() {
    ...
    if (state.lastActiveDate !== today) {
      ...
      // Check todayBlocks first (yesterday's count), then focusHistory as backup
      // in case todayBlocks was overwritten by a stale storage sync from another page.
      var _hadFocusYesterday = (state.todayBlocks || 0) > 0;
      if (!_hadFocusYesterday && state.lastActiveDate && state.focusHistory) {
        _hadFocusYesterday = !!(state.focusHistory[state.lastActiveDate] && state.focusHistory[state.lastActiveDate] > 0);
      }
      if (diffDays === 1 && _hadFocusYesterday) {
        ...
        state.realStreak = 0;
      } else if (diffDays === 1 && !_hadFocusYesterday) {
        state.realStreak = 0;
      }
  ...
  function renderGameLockout() {
    ...
    // v3.23.228: Family distress morse messages during lockout
    _maybeHouseholdMorse(locked);
  }

  // v3.23.228: Eerie household morse messages while locked out
  var _lastHouseholdMorseTime = 0;
  var _householdMorsePool = [
    { from: 'Spouse', text: 'PLEASE COME HOME' },
    { from: 'Spouse', text: 'WE MISS YOU' },
    { from: 'Spouse', text: 'THE LIGHTS KEEP FLICKERING' },
    { from: 'Spouse', text: 'IT IS WATCHING US' },
    { from: 'Spouse', text: 'ARE YOU STILL THERE' },
    { from: 'Spouse', text: 'DOORS WONT OPEN' },
    { from: 'Spouse', text: 'SOMETHING IS WRONG WITH THE HOUSE' },
    { from: 'Child', text: 'IM HUNGRY' },
    { from: 'Child', text: 'WHEN ARE YOU COMING BACK' },
    { from: 'Child', text: 'THE WALLS ARE HUMMING' },
    { from: 'Child', text: 'I HEAR SOMETHING IN THE WALLS' },
    { from: 'Child', text: 'IT WONT LET US LEAVE' },
    { from: 'Pet', text: 'BARK BARK BARK' },
    { from: 'Pet', text: 'WHIMPER' },
    { from: 'Unknown', text: 'HELP' },
    { from: 'Unknown', text: 'SOS' },
  ];
  var _householdMorseSent = [];
  function _maybeHouseholdMorse(locked) {
    if (!locked) { _lastHouseholdMorseTime = 0; return; }
    // Only if player has a house (spouse or kids or pets)
    if (!state.spouseCount && !state.kidCount && !state.petType) return;
    var now = Date.now();
    // Very rare — minimum 4 hours between messages, ~5% chance per eligible check
    var minGap = 4 * 60 * 60 * 1000;
    if (_lastHouseholdMorseTime === 0) { _lastHouseholdMorseTime = now; return; } // First lockout render — start the clock
    if (now - _lastHouseholdMorseTime < minGap) return;
    // ~5% chance each eligible render cycle — very sparse, maybe one every few days
    if (Math.random() > 0.05) return;
    _lastHouseholdMorseTime = now;
    // Pick a message we haven't sent recently
    var available = [];
    for (var hp = 0; hp < _householdMorsePool.length; hp++) {
      if (_householdMorseSent.indexOf(hp) === -1) available.push(hp);
    }
    if (available.length === 0) { _householdMorseSent = []; available = []; for (var hr = 0; hr < _householdMorsePool.length; hr++) available.push(hr); }
    var pick = available[Math.floor(Math.random() * available.length)];
    _householdMorseSent.push(pick);
    var msg = _householdMorsePool[pick];
    // Filter by what the player actually has
    if (msg.from === 'Child' && !state.kidCount) return;
    if (msg.from === 'Pet' && !state.petType) return;
    if (msg.from === 'Spouse' && !state.spouseCount) return;
    var fromName = msg.from === 'Pet' ? (state.petName || 'Your Pet') : (msg.from === 'Child' ? 'Your Child' : (msg.from === 'Spouse' ? 'Your Spouse' : '???'));
    var morseText = '';
    try { morseText = typeof MorseMessenger !== 'undefined' ? MorseMessenger.encode(msg.text) : msg.text; } catch(_) { morseText = msg.text; }
  ...
  function buildPipMarkup() {
    ...
    // Hamburger menu on the right opens a distractions panel that
    // slides down below the timer pill. Categories are tappable
    // buttons with counts. Scaling penalty deducts from session $.
    var cats = (state.distractionCategories && state.distractionCategories.length > 0)
      ? state.distractionCategories
      : [{id:'_other',name:'Other',color:'#888'}];
    // Always ensure "Other" is present
    var hasOther = false;
    for (var ci = 0; ci < cats.length; ci++) { if (cats[ci].id === '_other') hasOther = true; }
    if (!hasOther) cats.push({id:'_other',name:'Other',color:'#888'});
    ...
    if (state.sessionDistractions) {
      ...
      for (var di = 0; di < dk.length; di++) totalDist += state.sessionDistractions[dk[di]];
    }
    var penaltyPct = Math.min(10 * totalDist * (totalDist + 1) / 2, 100);
    var penaltyText = totalDist > 0 ? '-' + penaltyPct + '% penalty (' + totalDist + ' distraction' + (totalDist !== 1 ? 's' : '') + ')' : '';
    ...
    return '' +
      ...
      '  .pip-wrap{display:flex;flex-direction:column;height:100%;margin:4px;overflow:hidden;}' +
      '  .pip-card{position:relative;flex:0 0 auto;display:flex;align-items:center;gap:10px;padding:8px 14px 10px 10px;background:linear-gradient(180deg,#10101c 0%,#0a0a14 100%);border:1px solid #1f1f30;border-radius:18px;overflow:hidden;}' +
      ...
      '  .pip-menu-btn{flex:0 0 auto;width:28px;height:28px;border:none;background:transparent;color:#4ecdc4;cursor:pointer;padding:0;display:flex;align-items:center;justify-content:center;border-radius:6px;transition:transform 0.15s cubic-bezier(0.34,1.56,0.64,1),background 0.2s,box-shadow 0.2s,color 0.2s;font-size:18px;line-height:1;}' +
      ...
      '  .dist-cat:hover{transform:scale(1.08) translateY(-2px);box-shadow:0 0 24px rgba(255,255,255,0.25),0 6px 16px rgba(0,0,0,0.5);border-color:rgba(255,255,255,0.4);filter:brightness(1.2);}' +
      ...
      '  .dist-input-row{display:none;margin-top:6px;gap:4px;align-items:center;}' +
      '  .dist-input-row.open{display:flex;}' +
      '  .dist-input{flex:1;font-size:8px;padding:4px 6px;border:1px solid #333;border-radius:6px;background:#1a1a28;color:#4ecdc4;font-family:"Press Start 2P","Courier New",monospace;outline:none;}' +
      '  .dist-input:focus{border-color:#4ecdc4;}' +
      '  .dist-input-btn{font-size:7px;padding:4px 8px;border:1px solid #4ecdc4;border-radius:6px;background:#0a0a14;color:#4ecdc4;cursor:pointer;font-family:"Press Start 2P","Courier New",monospace;transition:transform 0.15s cubic-bezier(0.34,1.56,0.64,1),background 0.15s,box-shadow 0.2s;box-shadow:0 2px 0 rgba(0,0,0,0.3);}' +
      '  .dist-input-btn:hover{background:#1a2a28;transform:scale(1.06) translateY(-1px);box-shadow:0 0 10px rgba(78,205,196,0.3),0 3px 6px rgba(0,0,0,0.4);}' +
      '  .dist-input-btn:active{transform:scale(0.95) translateY(1px);box-shadow:0 1px 0 rgba(0,0,0,0.3);}' +
      '</style></head><body>' +
      '<div class="pip-wrap">' +
      '<div class="pip-card">' +
      ...
      '    <button class="dist-input-btn" id="distInputOk">OK</button>' +
      '  </div>' +
      '</div>' +
      '</div></body></html>';
  }
  ...
  // v3.23.312: Distraction penalty — percentage of session earnings.
  // 1st distraction = 10%, 2nd = +20% more (30% total), 3rd = +30% (60%), 4th = +40% (100%).
  // Each successive distraction adds 10% more than the previous increment.
  // Capped at 100%. Returns the percentage (0-100).
  function getDistractionPenaltyPct() {
    var n = getDistractionCount();
    if (n <= 0) return 0;
    // Increments: 10, 20, 30, 40, 50... (triangular)
    // Total for n distractions = sum of 10*k for k=1..n = 10 * n*(n+1)/2
    var pct = 10 * n * (n + 1) / 2;
    return Math.min(pct, 100);
  }

  // v3.23.312: Count total distractions in current session
  function getDistractionCount() {
    if (!state.sessionDistractions) return 0;
    var total = 0;
    var dk = Object.keys(state.sessionDistractions);
    for (var i = 0; i < dk.length; i++) total += state.sessionDistractions[dk[i]];
    return total;
  }
  ...
  // v3.23.311: Wire all distractions panel interactivity inside the PiP window.
  function _wireDistractionsPanel(win) {
    var doc = win.document;
    var menuBtn = doc.getElementById('pipMenuBtn');
    var panel = doc.getElementById('distPanel');
    ...
    function _refreshPanel() {
      var catsEl = doc.getElementById('distCats');
      var penaltyEl = doc.getElementById('distPenalty');
      if (!catsEl) return;

      var cats = (state.distractionCategories && state.distractionCategories.length > 0)
        ? state.distractionCategories.slice()
        : [];
      var hasOther = false;
      for (var i = 0; i < cats.length; i++) { if (cats[i].id === '_other') hasOther = true; }
      if (!hasOther) cats.push({id:'_other',name:'Other',color:'#888'});
      ...
      // Re-wire click handlers on new elements
      var catEls = catsEl.querySelectorAll('.dist-cat');
      ...
      if (penaltyEl) {
        ...
        if (state.sessionDistractions) {
          var dk = Object.keys(state.sessionDistractions);
          for (var d = 0; d < dk.length; d++) totalDist += state.sessionDistractions[dk[d]];
        }
    ...
    var inputRow = doc.getElementById('distInputRow');
    var inputEl = doc.getElementById('distInput');
    var inputOk = doc.getElementById('distInputOk');
    var _editMode = false;
    var _editingId = null;
    ...
    if (addBtn && inputRow && inputEl && inputOk) {
      addBtn.addEventListener('click', function(e) {
        ...
        if (inputRow.classList.contains('open')) inputEl.focus();
      });

      function _submitInput() {
        var name = inputEl.value.trim();
        if (!name) return;
        if (!state.distractionCategories) state.distractionCategories = [];
        if (_editMode && _editingId) {
          // Edit existing
          for (var i = 0; i < state.distractionCategories.length; i++) {
            if (state.distractionCategories[i].id === _editingId) {
              state.distractionCategories[i].name = name;
              break;
            }
          }
        } else {
          // Add new
          var id = 'dist_' + Date.now();
          var colorIdx = state.distractionCategories.length % DIST_COLORS.length;
          state.distractionCategories.push({ id: id, name: name, color: DIST_COLORS[colorIdx] });
        }
        save();
        inputEl.value = '';
    ...
    // EDIT button — toggles edit mode where clicking a category lets you rename/delete
    var editBtn = doc.getElementById('distEditBtn');
    ...
    function _refreshEditMode() {
      var catsEl = doc.getElementById('distCats');
      if (!catsEl || !_inEditMode) return;
      var catEls = catsEl.querySelectorAll('.dist-cat');
      for (var j = 0; j < catEls.length; j++) {
        var el = catEls[j];
        var catId = el.getAttribute('data-cat-id');
        if (catId === '_other') continue; // Can't edit/delete Other
        // Add delete X button if not already present
        if (!el.querySelector('.dist-del')) {
          var del = doc.createElement('span');
          del.className = 'dist-del';
          del.textContent = '✕';
          del.style.cssText = 'margin-left:4px;cursor:pointer;font-size:8px;color:rgba(255,255,255,0.7);';
          (function(cid, delEl) {
            delEl.addEventListener('click', function(ev) {
              ev.stopPropagation();
              if (!state.distractionCategories) return;
              state.distractionCategories = state.distractionCategories.filter(function(c) { return c.id !== cid; });
              // Also remove from session counts
              if (state.sessionDistractions && state.sessionDistractions[cid]) {
                delete state.sessionDistractions[cid];
              }
              save();
              _refreshPanel();
              if (_inEditMode) setTimeout(_refreshEditMode, 50);
            });
          })(catId, del);
          el.appendChild(del);
        }
        // Clicking the name in edit mode = rename
        var nameSpan = el.querySelector('.dist-cat-name');
        if (nameSpan && !nameSpan._editWired) {
          nameSpan._editWired = true;
          (function(cid, ns) {
            // Override the parent click in edit mode
            ns.addEventListener('click', function(ev) {
              if (!_inEditMode) return;
              ev.stopPropagation();
              ev.preventDefault();
              _editMode = true;
              _editingId = cid;
              inputEl.value = ns.textContent;
              inputEl.placeholder = 'Rename category';
    ...
    if (window.documentPictureInPicture && typeof window.documentPictureInPicture.requestWindow === 'function') {
      ...
      window.documentPictureInPicture.requestWindow({ width: 220, height: 78 })
        .then(function(win) {
          ...
          // v3.23.180: Remember pop-out was opened so we can restore after reload
          state.popOutTimerOpen = true;
          save();
          // When the user closes the PiP window manually (or the main
          // tab navigates), drop the handle so the next click re-opens.
          try {
            win.addEventListener('pagehide', function() { pipWindow = null; state.popOutTimerOpen = false; save(); });
            win.addEventListener('unload', function() { pipWindow = null; state.popOutTimerOpen = false; save(); });
          } catch (_) {}
          ...
          // v3.23.311: Wire distractions panel
          try { _wireDistractionsPanel(win); } catch (e) { console.error('Distractions panel wire failed:', e); }
          try { renderPopOutTimer(); } catch (_) {}
          try {
            if (typeof MsgLog !== 'undefined') {
  ...
  function awardSessionReward(reward) {
    ...
    // v3.23.312: Apply distraction penalty or focus bonus
    try {
      ...
      if (_distPenaltyPct > 0) {
        ...
        var _coinsThisSession = Math.max(0, (state.coins || 0) - (_celebSnapshotCoins || 0));
        var _distPenalty = Math.round(_coinsThisSession * _distPenaltyPct / 100);
        // Deduct from wallet (floor at 0)
        var _deducted = Math.min(_distPenalty, state.coins || 0);
        if (_deducted > 0) {
  ...
  function editDailyReminder(id, newText) {
    if (!newText || !Array.isArray(state.dailyReminders)) return;
    for (var i = 0; i < state.dailyReminders.length; i++) {
      if (state.dailyReminders[i].id === id) {
        state.dailyReminders[i].text = newText;
        break;
      }
    }
    save();
  ...
  function renderDailyReminders() {
    ...
    items.forEach(function(r) {
      ...
      var txt = document.createElement('div');
      txt.style.cssText = 'flex:1;color:#b0d4ff;font-size:12px;word-break:break-word;cursor:pointer;';
      ...
      txt.addEventListener('click', function() {
        ...
        input.type = 'text';
        input.value = r.text;
        input.style.cssText = 'flex:1;background:#0a1a2a;color:#b0d4ff;border:1px solid #4ecdc4;border-radius:4px;padding:4px 6px;font-size:12px;font-family:inherit;outline:none;width:100%;';
        ...
        function _saveEdit() {
          ...
          if (val && val !== r.text) {
            editDailyReminder(r.id, val);
          } else {
            renderDailyReminders(); // revert
          }
        }
        input.addEventListener('keydown', function(e) {
          if (e.key === 'Enter') { e.preventDefault(); _saveEdit(); }
          if (e.key === 'Escape') { e.preventDefault(); renderDailyReminders(); }
        });
        input.addEventListener('blur', _saveEdit);
      });

      var editBtn = document.createElement('button');
      editBtn.type = 'button';
      editBtn.textContent = '\u270E';
      editBtn.title = 'Edit this reminder';
      editBtn.style.cssText = 'background:transparent;color:#4ecdc4;border:1px solid #2a6090;border-radius:4px;padding:3px 7px;font-size:10px;cursor:pointer;';
      editBtn.addEventListener('click', function() { txt.click(); });
  ...
  function init() {
    ...
    load(function() {
      ...
      buildShuffledTabOrder();
      // v3.23.187: Restore grace period from persisted state so it survives popup close/reopen
      if (state.gameLockGraceUntil && state.gameLockGraceUntil > Date.now()) {
        _gameLockGraceUntil = state.gameLockGraceUntil;
      }
      render();
      ...
      // v3.23.186: Auto-restore pop-out timer after reload by programmatically
      // clicking the pop-out button (counts as user gesture for PiP API).
      if (state.popOutTimerOpen && (state.timerState === 'running' || state.timerState === 'paused' || state.timerState === 'countdown')) {
        setTimeout(function() {
          var _popBtn = document.getElementById('popOutTimerBtn');
          if (_popBtn) _popBtn.click();
        }, 800);
      } else if (state.popOutTimerOpen) {
    ...
    setTimeout(function() {
      try {
        ...
        function _checkAllBadges() {
          ...
          var reqs = [
            ...
            ['morseSent',state.morseSentCount || 0],
            ['morseRecv',state.morseReceivedCount || 0],
            ['famousCount',(state.morseFamousSent && state.morseFamousSent.length) || 0]
          ];
          var valMap = {};
          ...
          // v3.23.240: Famous morse message badges
          var _fms = state.morseFamousSent || [];
          valMap['famousSos'] = _fms.indexOf('sos') !== -1 ? 1 : 0;
          valMap['famousHello'] = _fms.indexOf('hello') !== -1 ? 1 : 0;
          valMap['famousCqd'] = _fms.indexOf('cqd') !== -1 ? 1 : 0;
          valMap['famousWhgw'] = _fms.indexOf('whgw') !== -1 ? 1 : 0;
          valMap['famousWatson'] = _fms.indexOf('watson') !== -1 ? 1 : 0;
          valMap['famousTogo'] = _fms.indexOf('togo') !== -1 ? 1 : 0;
          valMap['famousMoon'] = _fms.indexOf('moon') !== -1 ? 1 : 0;
          valMap['famousStop'] = _fms.indexOf('stop') !== -1 ? 1 : 0;
          valMap['famousStein'] = _fms.indexOf('stein') !== -1 ? 1 : 0;
      ...
      // v3.23.183: Listen for play/pause messages from timer-popout.html
      try {
        chrome.runtime.onMessage.addListener(function(msg) {
          if (msg && msg.type === 'popout_toggle_timer') {
      ...
      (function initFriendsUI() {
        ...
        // v3.23.241: Morse demo mode for tutorial — opens overlay without a real friend
        window._openMorseDemo = function() {
          if (typeof MorseMessenger !== 'undefined') {
            MorseMessenger.open('_tutorial_demo', 'Tutorial Demo', state.profileId || 'demo', function() {
              // No-op send — demo mode doesn't actually transmit
              try { notify('Demo mode — messages are not sent', '#ffd700'); } catch(_) {}
            });
          }
        };
        ...
        function renderMorseInbox() {
          ...
          var empty = document.getElementById('morseInboxEmpty');
          var countBadge = document.getElementById('morseInboxCount');
          var clearBtn = document.getElementById('morseInboxClearBtn');
          var header = document.getElementById('morseInboxHeader');
...
/* ═══════════════════════════════════════════════════════════════════════
   TUTORIAL ENCYCLOPEDIA — Searchable help catalog
   Click the graduation cap button → searchable encyclopedia of all features.
   Categories gated by actual building purchases, not tiers.
   Locked categories show redacted names. HMAC codes can force-unlock.
   ═══════════════════════════════════════════════════════════════════════ */
(function() {
  console.log('[TUT-DEBUG] Tutorial IIFE starting...');
  var _tutBtn = document.getElementById('tutorialBtn');
  var _overlay = document.getElementById('tutorialOverlay');
  var _backdrop = document.getElementById('tutorialBackdrop');
  var _catalog = document.getElementById('tutCatalog');
  var _catalogBody = document.getElementById('tutCatalogBody');
  var _catalogClose = document.getElementById('tutCatalogClose');
  var _searchInput = document.getElementById('tutSearchInput');
  var _codeEntry = document.getElementById('tutCodeEntry');
  var _codeInput = document.getElementById('tutCodeInput');
  var _codeSubmit = document.getElementById('tutCodeSubmit');
  var _codeCancel = document.getElementById('tutCodeCancel');
  var _codeError = document.getElementById('tutCodeError');
  var _codeTierLabel = document.getElementById('tutCodeTierLabel');
  console.log('[TUT-DEBUG] Elements found:', {
    tutBtn: !!_tutBtn,
    overlay: !!_overlay,
    backdrop: !!_backdrop,
    catalog: !!_catalog,
    catalogBody: !!_catalogBody,
    catalogClose: !!_catalogClose,
    searchInput: !!_searchInput,
    codeEntry: !!_codeEntry,
    codeInput: !!_codeInput,
    codeSubmit: !!_codeSubmit,
    codeCancel: !!_codeCancel,
    codeError: !!_codeError,
    codeTierLabel: !!_codeTierLabel,
    TUTORIAL_CATEGORIES: !!(window.TUTORIAL_CATEGORIES),
    catCount: (window.TUTORIAL_CATEGORIES || []).length
  });
  if (!_tutBtn || !_overlay) {
    console.error('[TUT-DEBUG] BAILING OUT — missing tutBtn=' + !!_tutBtn + ' overlay=' + !!_overlay);
    return;
  }

  var _CATS = window.TUTORIAL_CATEGORIES || [];

  /* ── Building-based gate check ─────────────────────────────────── */
  function _isCatUnlocked(cat) {
    if (!cat.gate) return true;
    // HMAC force-unlock
    if (state.tutorialUnlocked && state.tutorialUnlocked[cat.id]) return true;
    switch (cat.gate) {
      case 'factory':       return !!state.hasSeenFactoryIntro;
      case 'market':        return !!(state.marketingLevel && state.marketingLevel > 0);
      case 'employees':     return !!(state.employeesLevel && state.employeesLevel > 0);
      case 'brokerage':     return !!state.brokerageUnlocked;
      case 'house':         return !!state.hasSeenFactoryIntro;
      case 'ratiocinatory': return !!state.ratiocinatoryUnlocked;
      case 'bureau':        return !!(state.bureauAgents && state.bureauAgents.length > 0);
      case 'research':      return !!(state.complianceFrameworkLevel && state.complianceFrameworkLevel > 0);
      case 'incinerator':   return !!state.materialsIncineratorUnlocked;
      case 'esoteric':      return !!(state.complianceFrameworkLevel && state.complianceFrameworkLevel >= 3);
      default:              return false;
    }
  }

  /* ── HMAC-SHA256 code verification ─────────────────────────────── */
  function _hmacVerify(catId, salt, inputCode, callback) {
    var pid = state.profileId || '';
    if (!pid) { callback(false); return; }
    var msg = catId + salt;
    var enc = new TextEncoder();
    crypto.subtle.importKey('raw', enc.encode(pid), { name: 'HMAC', hash: 'SHA-256' }, false, ['sign'])
      .then(function(key) {
      ...
      .then(function(sig) {
        ...
        var expected = hex.slice(0, 4).toUpperCase() + '-' + hex.slice(4, 8).toUpperCase();
        callback(inputCode.toUpperCase().trim() === expected);
      })
  ...
  function _highlightText(text, query) {
    ...
    return text.replace(new RegExp('(' + esc + ')', 'gi'), '<span class="tut-highlight">$1</span>');
  }

  /* ── Catalog rendering ─────────────────────────────────────────── */
  var _pendingUnlockCat = null;
  var _searchQuery = '';

  function _renderCatalog() {
    _catalogBody.innerHTML = '';
    _codeEntry.style.display = 'none';
    _codeError.style.display = 'none';
    _codeInput.value = '';
    ...
    for (var c = 0; c < _CATS.length; c++) {
      var cat = _CATS[c];
      var unlocked = _isCatUnlocked(cat);
      ...
      if (q) {
        ...
        matchedItems = cat.items;
      }
      anyResults = true;

      var section = document.createElement('div');
      section.className = 'tut-cat';

      var header = document.createElement('div');
      header.className = 'tut-cat-header' + (unlocked ? '' : ' locked');
      var catIcon = '<span class="cat-icon">' + cat.icon + '</span>';
      var catName = unlocked ? cat.name : _redactName(cat.name);
      var catCount = '<span class="cat-count">' + matchedItems.length + '</span>';
      var catArrow = '<span class="cat-arrow">▸</span>';
      if (unlocked) {
        header.innerHTML = catIcon + '<span class="cat-name">' + (q ? _highlightText(cat.name, q) : cat.name) + '</span>' + catCount + catArrow;
      } else {
        header.innerHTML = catIcon + '<span class="cat-name">' + catName + '</span><span class="cat-count" style="color:#ffd700;font-size:6px;">ENTER CODE</span>' + catArrow;
      }

      // Toggle expand/collapse
      (function(sec, hdr, catObj, isUnlocked, items) {
        hdr.addEventListener('click', function() {
          if (!isUnlocked) {
            _showCodeEntry(catObj);
            return;
          }
          var body = sec.querySelector('.tut-cat-body');
          if (body) {
            var wasOpen = body.style.display !== 'none';
            body.style.display = wasOpen ? 'none' : 'block';
            hdr.querySelector('.cat-arrow').textContent = wasOpen ? '▸' : '▾';
          }
        });
      })(section, header, cat, unlocked, matchedItems);

      section.appendChild(header);

      // Items body
      var body = document.createElement('div');
      body.className = 'tut-cat-body';
      body.style.display = q ? 'block' : 'none'; // auto-expand when searching

      if (unlocked) {
        for (var j = 0; j < matchedItems.length; j++) {
          var feat = matchedItems[j];
          var row = document.createElement('div');
          row.className = 'tut-feat';
          row.innerHTML = '<div class="tut-feat-label">' + (q ? _highlightText(feat.label, q) : feat.label) + '</div>';

          // Expandable description
          (function(r, f) {
            var descDiv = document.createElement('div');
            descDiv.className = 'tut-feat-desc';
            descDiv.style.display = 'none';
            descDiv.innerHTML = q ? _highlightText(f.desc, q) : f.desc;
            r.appendChild(descDiv);
            r.addEventListener('click', function() {
              var wasOpen = descDiv.style.display !== 'none';
              descDiv.style.display = wasOpen ? 'none' : 'block';
              r.classList.toggle('expanded', !wasOpen);
            });
          })(row, feat);
        ...
        for (var k = 0; k < matchedItems.length; k++) {
          ...
          lockedRow.className = 'tut-feat locked';
          lockedRow.innerHTML = '<div class="tut-feat-label" style="filter:blur(3px);user-select:none;">' + matchedItems[k].label + '</div>';
      ...
      _catalogBody.appendChild(section);
    }

    if (!anyResults && q) {
      var noRes = document.createElement('div');
      noRes.className = 'tut-no-results';
      noRes.textContent = 'No features found for "' + _searchQuery + '"';
      _catalogBody.appendChild(noRes);
    }
  }

  function _showCodeEntry(catObj) {
    _pendingUnlockCat = catObj;
    _codeEntry.style.display = 'block';
    _codeError.style.display = 'none';
    _codeInput.value = '';
    _codeTierLabel.textContent = 'This category requires an unlock code. Ask your tutorial guide for the code for your profile.';
    _codeInput.focus();
  }

  /* ── Catalog open/close ─────────────────────────────────────────── */
  function _openCatalog() {
    console.log('[TUT-DEBUG] _openCatalog() called');
    _searchQuery = '';
    if (_searchInput) _searchInput.value = '';
    _renderCatalog();
    console.log('[TUT-DEBUG] overlay classes before:', _overlay.className);
    _overlay.classList.add('active');
    _backdrop.style.display = 'block';
    _catalog.classList.add('active');
    console.log('[TUT-DEBUG] overlay classes after:', _overlay.className);
    console.log('[TUT-DEBUG] overlay display:', window.getComputedStyle(_overlay).display);
    console.log('[TUT-DEBUG] overlay visibility:', window.getComputedStyle(_overlay).visibility);
    console.log('[TUT-DEBUG] overlay z-index:', window.getComputedStyle(_overlay).zIndex);
  }

  function _closeCatalog() {
    _overlay.classList.remove('active');
    _catalog.classList.remove('active');
    _backdrop.style.display = 'none';
  }

  /* ── Code submission ────────────────────────────────────────────── */
  _codeSubmit.addEventListener('click', function() {
    if (_pendingUnlockCat === null) return;
    var code = _codeInput.value;
    if (!code || code.length < 4) {
      _codeError.style.display = 'block';
      return;
    }
    _hmacVerify(_pendingUnlockCat.id, _pendingUnlockCat.salt, code, function(valid) {
      if (valid) {
        _unlockCat(_pendingUnlockCat.id);
        _pendingUnlockCat = null;
        _renderCatalog();
      } else {
        _codeError.style.display = 'block';
        _codeInput.style.borderColor = '#ff6b6b';
        setTimeout(function() { _codeInput.style.borderColor = '#5a5a7e'; }, 1500);
      }
    });
  });
  _codeInput.addEventListener('keydown', function(e) {
    if (e.key === 'Enter') _codeSubmit.click();
  });
  _codeCancel.addEventListener('click', function() {
    _codeEntry.style.display = 'none';
    _pendingUnlockCat = null;
  });
