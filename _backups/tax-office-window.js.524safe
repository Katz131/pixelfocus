...
(function() {
  ...
  function _eugeneDiscount() {
    // Each lesson reduces effective tax rate cumulatively
    var owned = state.eugeneLessons || [];
    var reduction = 0;
    for (var i = 0; i < EUGENE_LESSONS.length; i++) {
      if (owned.indexOf(EUGENE_LESSONS[i].id) !== -1) {
  ...
  function _eugeneShouldReveal(statId) {
    ...
    var owned = state.eugeneLessons || [];
    for (var i = 0; i < EUGENE_LESSONS.length; i++) {
      if (owned.indexOf(EUGENE_LESSONS[i].id) !== -1 && EUGENE_LESSONS[i].reveals) {
  ...
  function renderEugene() {
    ...
    for (var i = 0; i < EUGENE_LESSONS.length; i++) {
      ...
      var bg = isDone ? '#4ecdc4' : (isNext ? '#ffd700' : '#222');
      var border = isNext ? '2px solid #fff' : (isDone ? '1px solid #3a8a80' : '1px solid #333');
      html += '<div style="flex:1;height:10px;background:' + bg + ';border-radius:4px;border:' + border + ';transition:all 0.15s;" title="' + EUGENE_LESSONS[i].title + (isDone ? ' (completed)' : (isNext ? ' (next)' : ' (locked)')) + '"></div>';
    }
    html += '</div>';
    ...
    for (var i = 0; i < EUGENE_LESSONS.length; i++) {
      ...
      if (isDone) {
        html += '<div class="eugene-lesson-cost owned">COMPLETED</div>';
      } else {
      ...
      if (isDone) {
        ...
        html += '<div class="eugene-lesson-body" style="border-left:2px solid var(--accent-dim);padding-left:10px;margin-left:2px;">"' + lesson.teaches + '"<div style="font-size:10px;color:var(--text-dim);margin-top:4px;">— Eugene Mercer, CPA</div></div>';
        html += '<div class="eugene-lesson-benefit">' + lesson.benefit + '</div>';
      } else if (isNext && !state.eugeneFired && !state.eugeneDeathTriggered) {
        // Show description + buy button
    ...
    if (owned.length >= 10 && !state.eugeneRetirementChoice) {
